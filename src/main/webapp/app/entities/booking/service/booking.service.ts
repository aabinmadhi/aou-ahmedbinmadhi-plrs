...
type RestOf<T extends IBooking | NewBooking> = Omit<T, 'startTime' | 'endTime'> & {
...
export class BookingService {
  ...
  protected convertDateFromClient<T extends IBooking | NewBooking | PartialUpdateBooking>(booking: T): RestOf<T> {
    return {
      ...booking,
      startTime: booking.startTime?.toJSON() ?? null,
      endTime: booking.endTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBooking: RestBooking): IBooking {
    return {
      ...restBooking,
      startTime: restBooking.startTime ? dayjs(restBooking.startTime) : undefined,
      endTime: restBooking.endTime ? dayjs(restBooking.endTime) : undefined,
    };
  }
