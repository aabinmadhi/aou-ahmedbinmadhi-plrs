...
class ProfitResourceIT {
    ...
    void createProfit() throws Exception {
        ...
        // Create the Profit
        ProfitDTO profitDTO = profitMapper.toDto(profit);
        restProfitMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(profitDTO)))
    ...
    void putExistingProfit() throws Exception {
        ...
        updatedProfit
            ...
            .userTotalProfit(UPDATED_USER_TOTAL_PROFIT)
            .plrsTotalProfit(UPDATED_PLRS_TOTAL_PROFIT);
        ProfitDTO profitDTO = profitMapper.toDto(updatedProfit);

        restProfitMockMvc
            .perform(
                put(ENTITY_API_URL_ID, profitDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(profitDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamProfit() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProfitMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(profitDTO)))
    ...
    void patchNonExistingProfit() throws Exception {
        ...
        restProfitMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, profitDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(profitDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamProfit() throws Exception {
        ...
        restProfitMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(profitDTO))
            )
            .andExpect(status().isMethodNotAllowed());
