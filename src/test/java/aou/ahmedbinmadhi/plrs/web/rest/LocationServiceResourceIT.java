...
class LocationServiceResourceIT {
    ...
    void createLocationService() throws Exception {
        ...
        LocationServiceDTO locationServiceDTO = locationServiceMapper.toDto(locationService);
        restLocationServiceMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(locationServiceDTO))
            )
    ...
    void putExistingLocationService() throws Exception {
        ...
        updatedLocationService
            ...
            .printer(UPDATED_PRINTER)
            .city(UPDATED_CITY);
        LocationServiceDTO locationServiceDTO = locationServiceMapper.toDto(updatedLocationService);

        restLocationServiceMockMvc
            .perform(
                put(ENTITY_API_URL_ID, locationServiceDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(locationServiceDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamLocationService() throws Exception {
        ...
        restLocationServiceMockMvc
            .perform(
                put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(locationServiceDTO))
            )
    ...
    void patchNonExistingLocationService() throws Exception {
        ...
        restLocationServiceMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, locationServiceDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(locationServiceDTO))
            )
            .andExpect(status().isBadRequest());
